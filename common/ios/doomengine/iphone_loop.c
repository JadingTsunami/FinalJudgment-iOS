...
void iphoneDrawScreen() {
	...
	if ( automapmode & am_active ) {
		...
		if ( gamestate == GS_FINALE ) {
			...
			// next episode
            /* JDS: Show menu button, but in a static location
             *      This fixes a bug where the menu button overlaps with
             *      intermission screen text
             */
            static ibutton_t btnFinaleMenu;
            
            if ( !btnFinaleMenu.texture ) {
                SetButtonPicsAndSizes( &btnFinaleMenu, "iphone/menu_button.tga", "", 480-48, 0, 48, 48 );
            }
            
			if ( HandleButton( &btnFinaleMenu ) ) {
				iphonePauseMusic();
				menuState = IPM_MAIN;
                iphoneMainMenu();
			}
